...
public partial class VerAbonos : ContentPage
{
    ...
    private async void llenarAbonos(string lngNumeroUniCre, string strNombreCliente, string saldo)
    {
        lstMovimientos.ItemsSource = null;
        MovimientosCollection.Clear();
        ...
        try
        {
            if (CONEXIONMAESTRA.VerificarCon())
            {
                using (SqlConnection conn = CONEXIONMAESTRA.GetConnection())
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("FiltrarRegistrosDePagosDelCredito", conn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@lngConsecutCre", lngNumeroUniCre);
                ...
                if (MovimientosCollection.Any())
                {
                    lstMovimientos.ItemsSource = MovimientosCollection;

                    var abonos = MovimientosCollection
                        .Where(m => m.strCodTipMov == "AB")
                        .ToList();
            ...
            {
                await DisplayAlert("Sin Internet", "Está trabajando sin Internet (53)", "OK");
            }
        }
        catch (Exception ex)
        {
